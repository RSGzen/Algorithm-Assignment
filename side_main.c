...
void mergeByRating(int left, int mid, int right, int* temp_id, char** temp_name, int* temp_price, int* temp_rating, int* temp_dist){
    ...
    // Temp arrays
    int L_id[n1], R_id[n2];
    int L_price[n1], R_price[n2];
    int L_rating[n1], R_rating[n2];
    int L_dist[n1], R_dist[n2];
    char *L_name[n1], *R_name[n2];
    ...
    for (int i = 0; i < n1; i++) free(L_name[i]);
    for (int j = 0; j < n2; j++) free(R_name[j]);
}
...
int binarySearchByName(char** temp_name, int current_num_hotels, char* target_name) {
    ...
    while (left <= right) {
        int mid = left + (right - left) / 2;
        int cmp = strcasecmp(temp_name[mid], target_name);
...
void mergeByName(
    ...
    while (i < n1 && j < n2) {
        if (strcasecmp(L_name[i], R_name[j]) <= 0) {
            id[k] = L_id[i];
            price[k] = L_price[i];
            rating[k] = L_rating[i];
            dist[k] = L_dist[i];
